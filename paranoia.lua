...
local notifications=(function()local table_insert,table_remove=table.insert,table.remove;local renderer_rectangle,renderer_text,renderer_measure_text=renderer.rectangle,renderer.text,renderer.measure_text;local math_min,math_max=math.min,math.max;local globals_realtime=globals.realtime;local client_screen_size=client.screen_size;local notifications={active={},settings={position=1,duration=5,max_notifications=7,fade_time=0.3,spacing=5,width=350,title_size=14,text_size=12,background_color={17,17,17,200},text_color={200,200,200,255},accent_color={124,195,13,255}}};local POSITIONS={[1]='top-right',[2]='top-left',[3]='bottom-right',[4]='bottom-left',[5]='bottom-center'};local function calculate_notification_size(notification)local settings=notifications.settings;local title_width,title_height=renderer_measure_text('b',0,notification.title);local text_width,text_height=0,0;if notification.text and notification.text~=''then text_width,text_height=renderer_measure_text('',0,notification.text);notification.height=title_height+text_height+40 else notification.height=title_height+28 end;notification.title_height=title_height;notification.text_height=text_height end;function notifications.add(title,text,pos,duration,color)local settings=notifications.settings;local position=settings.position;if type(pos)=='number'and pos>=1 and pos<=5 then position=pos end;local notification={title=title or'title',text=text or'',position=position,creation_time=globals_realtime(),duration=duration or settings.duration,alpha=0,height=0,y_offset=0,color=color or settings.accent_color};calculate_notification_size(notification);if#notifications.active>=settings.max_notifications then table_remove(notifications.active,1)end;table_insert(notifications.active,notification);return notification end;local function get_notification_position(notification,total_height)local screen_width,screen_height=client_screen_size();local settings=notifications.settings;local width=settings.width;local height=notification.height;local position=notification.position;local position_handlers={[1]=function()return screen_width-width-10,10+total_height end,[2]=function()return 10,10+total_height end,[3]=function()return screen_width-width-10,screen_height-height-10-total_height end,[4]=function()return 10,screen_height-height-10-total_height end,[5]=function()return(screen_width-width)/2,screen_height-height-10-total_height end};local handler=position_handlers[position]or position_handlers[1];return handler()end;local function group_notifications_by_position()local grouped={};for _,notification in ipairs(notifications.active)do local position=notification.position;grouped[position]=grouped[position]or{};table_insert(grouped[position],notification)end;return grouped end;local function calculate_colors(notification,alpha)local settings=notifications.settings;local bg=settings.background_color;local bg_color={bg[1],bg[2],bg[3],bg[4]*alpha};local accent=notification.color;local accent_color={accent[1],accent[2],accent[3],accent[4]*alpha};local text=settings.text_color;local text_color={text[1],text[2],text[3],text[4]*alpha};return bg_color,accent_color,text_color end;local function update_notification_alpha(notification,current_time)local settings=notifications.settings;local time_delta=current_time-notification.creation_time;local time_left=notification.duration-time_delta;if time_delta<settings.fade_time then notification.alpha=time_delta/settings.fade_time elseif time_left<settings.fade_time then notification.alpha=time_left/settings.fade_time else notification.alpha=1 end;return time_left<=0 end;local function on_paint()if#notifications.active==0 then return end;local settings=notifications.settings;local current_time=globals_realtime();for i=#notifications.active,1,-1 do local notification=notifications.active[i];local should_remove=update_notification_alpha(notification,current_time);if should_remove then table_remove(notifications.active,i)end end;local grouped=group_notifications_by_position();for position,notifications_group in pairs(grouped)do local total_height=0;for i,notification in ipairs(notifications_group)do notification.y_offset=total_height;total_height=total_height+notification.height+settings.spacing end;for _,notification in ipairs(notifications_group)do local alpha=notification.alpha;if alpha>0 then local x,y=get_notification_position(notification,notification.y_offset);local width=settings.width;local height=notification.height;local bg_color,accent_color,text_color=calculate_colors(notification,alpha);renderer_rectangle(x,y,width,height,bg_color[1],bg_color[2],bg_color[3],bg_color[4]);renderer_rectangle(x,y,4,height,accent_color[1],accent_color[2],accent_color[3],accent_color[4]);renderer_text(x+12,y+8,text_color[1],text_color[2],text_color[3],text_color[4],'b',0,notification.title);if notification.text and notification.text~=''then renderer_text(x+12,y+8+notification.title_height+15,text_color[1],text_color[2],text_color[3],text_color[4],'',0,notification.text)end end end end end;client.set_event_callback('paint_ui',on_paint);client.set_event_callback('shutdown',function()if notifications.settings then for _,notification in ipairs(notifications.settings)do notification=nil end end;collectgarbage('collect');collectgarbage('collect')end);return notifications end)()
